...
trimData <- function(object,
  ...
  if (uniform) {
    delta <-
      abs(object@data$x[start.idx + 1] - object@data$x[start.idx])
...
dft <- function(object) {
  ...
  times <- object@data$x
  trimmed@data$x <-
    time2invcm(times)[1:(ceiling(length(f@data$x) / 2) + 1)]
  trimmed@label$x <- expression(Frequency (cm ^ {
    -1
  }))
  return(trimmed)
}
